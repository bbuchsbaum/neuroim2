...
```{r}
      all.equal(sphere, rep(100, length(sphere)))   
```
...
slist <- searchlight(vol, radius=8)
...
Another type of ROI analysis, similar to the 'searchlight', involves working with square of cuboid image "patches". The `patch_set` function can be used to generate a set of equally-sized patches that span the image space (or some mask covering the space). While a 'searchlight' may have different sizes depending on it's surround, patches are guranteed to be of equal size. This means that at edges, 'patches' will be padded out with the value at the image extremes.
