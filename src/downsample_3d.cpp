...
// [[Rcpp::export]]
NumericVector downsample_3d_cpp(NumericVector arr, 
                                IntegerVector new_dims,
                                IntegerVector old_dims) {
  
  // Validate dimension vectors
  if (old_dims.size() != 3 || new_dims.size() != 3) {
    stop("Both old_dims and new_dims must have exactly 3 elements");
  }
  
  // old_dims and new_dims are both 3D (x, y, z)
  ...
  R_xlen_t expected_size = (R_xlen_t)old_nx * old_ny * old_nz;
  ...
  // Create output array
  R_xlen_t out_size = (R_xlen_t)new_nx * new_ny * new_nz;
  ...
  for (int new_z = 0; new_z < new_nz; new_z++) {
    for (int new_y = 0; new_y < new_ny; new_y++) {
      for (int new_x = 0; new_x < new_nx; new_x++) {
        ...
        for (int z = z_min; z < z_max; z++) {
          for (int y = y_min; y < y_max; y++) {
            for (int x = x_min; x < x_max; x++) {
              ...
              R_xlen_t old_idx = x + y * old_stride_y + z * old_stride_z;
        ...
        R_xlen_t new_idx = new_x + new_y * new_stride_y + new_z * new_stride_z;
