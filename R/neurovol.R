...
ClusteredNeuroVol <- function(mask, clusters, labelMap=NULL, label="") {
  ...
  new("ClusteredNeuroVol", mask=mask, clusters=as.integer(clusters), labelMap=labelMap, clusterMap=clusterMap, space=space)
}
...
## TODO reduce code duplication with load_data#BrainVectorSource
...
setMethod(f="coordToGrid", signature=signature(x="NeuroVol", coords="matrix"),
...
setMethod(f="patchSet", signature=signature(x="NeuroVol", 
                                            dims="numeric", 
...
#' @export
setMethod(f="numClusters", signature=signature(x="ClusteredNeuroVol"), 
          def=function(x) {
            length(x@clusterMap)
          })
...
setMethod(f="connComp", signature=signature(x="NeuroVol"), 
...
setMethod(f="writeVolume",signature=signature(x="NeuroVol", file_name="character", format="missing", data_type="missing"),
		def=function(x, file_name) {
			write.nifti.volume(x, file_name)           
		})

 
#' @export
#' @rdname writeVolume-methods
setMethod(f="writeVolume",signature=signature(x="ClusteredNeuroVol", file_name="character", format="missing", data_type="missing"),
          def=function(x, file_name) {
...
#' @export
#' @rdname writeVolume-methods
setMethod(f="writeVolume",signature=signature(x="NeuroVol", file_name="character", format="character", data_type="missing"),
...
#' @export writeVolume
#' @rdname writeVolume-methods
setMethod(f="writeVolume",signature=signature(x="NeuroVol", file_name="character", format="missing", data_type="character"),
		def=function(x, file_name, data_type) {
			write.nifti.volume(x, file_name, data_type)   
