...
bootstrap_searchlight <- function(mask, radius=8, iter=100) {
  ...
  obj <- list(nextElem=nextEl, progress=prog)
  class(obj) <- c("bootstrap_searchlight", "searchlight", 'abstractiter', 'iter')
  obj
...
searchlight <- function(mask, radius, eager=FALSE) {
  ...
  if (eager) {
    cds <- index_to_coord(mask, as.numeric(mask.idx))
...
#' @return an \code{iter} class 
#' @export
clustered_searchlight <- function(mask, csize) {
  mask.idx <- which(mask != 0)
  grid <- index_to_coord(mask, as.numeric(mask.idx))
  vox <- index_to_grid(mask, as.numeric(mask.idx))
  ...
  class(obj) <- c("clustered_searchlight", "searchlight", 'abstractiter', 'iter')
  obj
