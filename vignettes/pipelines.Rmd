...
This article focuses on the direct iteration tools in `neuroim2`: splitting
volumes, vectors, and voxel series into pieces, mapping ordinary R functions
over those pieces, and reducing the results. For ROI- and searchlight-centered
workflows, see `vignette("AnalysisWorkflows")`; for slice-, volume-, and
voxel-vector iteration, start here.
...
The examples below use `purrr` and simple list-processing. They are useful when
the pieces are defined directly by the image container itself: slices from a
`NeuroVol`, volumes from a `NeuroVec`, or voxel-wise time series from a 4D
object.
