...
## Read volumes to establish context
...
are specific to 3D work. We use the binary mask volume for mask examples and
the anatomical volume for slice visualization, where intensity variation matters.
...
```{r}
    vol2 <- NeuroVol((manual_vol + 1) * 25, space(manual_vol))
    max(vol2)
    space(vol2)
```
...
The easiest way to view an intensity volume is with `plot()`, which shows a
3 x 3 montage of evenly-spaced axial slices. Use an anatomical or statistical
image here rather than a binary mask or a synthetic all-zero array:
...
```{r slice_mid, fig.alt='Mid-slice of example volume (grayscale image).', fig.cap='Mid-slice of example volume'}
    z <- ceiling(dim(anat_vol)[3] / 2)
    image(anat_vol[, , z], main = paste("Slice z=", z), col = gray.colors(256))
```
