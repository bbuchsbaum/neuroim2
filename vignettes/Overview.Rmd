...
mask_data <- array(0, dim(img))
mask_data[20:40, 20:40, 15:25] <- 1
mask_vol <- LogicalNeuroVol(mask_data > 0, space(img))

# Convert to sparse representation
sparse_mask <- as.sparse(mask_vol)
cat("Dense size:", object.size(mask_vol), "bytes\n")
cat("Sparse size:", object.size(sparse_mask), "bytes\n")
cat("Compression ratio:", 
    round(as.numeric(object.size(mask_vol)) / as.numeric(object.size(sparse_mask)), 1), 
    "x\n")
```
