...
```{r}
# Create a regular volume with sparse data
vol_data <- array(0, dim(img))
vol_data[20:40, 20:40, 15:25] <- rnorm(21*21*11)
regular_vol <- NeuroVol(vol_data, space(img))

# Create a mask for the non-zero region
mask_vol <- LogicalNeuroVol(vol_data != 0, space(img))

# Convert to sparse representation using the mask
sparse_vol <- as.sparse(regular_vol, mask_vol)
cat("Dense size:", format(object.size(regular_vol), units = "auto"), "\n")
cat("Sparse size:", format(object.size(sparse_vol), units = "auto"), "\n")
cat("Non-zero voxels:", sum(mask_vol), "out of", prod(dim(mask_vol)), "\n")
```
...
```{r}
# Gaussian smoothing
img_smooth <- gaussian_blur(img, sigma = c(2, 2, 2))
