...
test_that("simulate_fmri spatial smoothing affects neighboring voxels", {
  dims <- c(12, 12, 8)
  mask_array <- array(TRUE, dims)
  mask <- NeuroVol(mask_array, NeuroSpace(dims, c(2, 2, 2)))
  ...
  sim_smooth <- simulate_fmri(
    mask,
    n_time = n_time,
    spatial_fwhm = 8,  # High smoothing
    seed = 444
  )
  
  # Low spatial smoothing
  sim_rough <- simulate_fmri(
    mask,
    n_time = n_time,
    spatial_fwhm = 1,  # Low smoothing
    seed = 444  # Same seed for comparison
  )
  
  # Calculate spatial correlation at one timepoint
  t <- 25
  smooth_vol <- as.array(sim_smooth)[,,,t]
  rough_vol <- as.array(sim_rough)[,,,t]
  
  # Check correlation between neighboring voxels
  # Smooth data should have higher correlation
  smooth_corr <- numeric(10)
  rough_corr <- numeric(10)
  
  for (i in 1:10) {
    # Random interior voxel
    x <- sample(2:(dims[1]-1), 1)
    y <- sample(2:(dims[2]-1), 1)
    z <- sample(2:(dims[3]-1), 1)
    
    # Get center and neighbor values
    center_smooth <- smooth_vol[x, y, z]
    neighbor_smooth <- smooth_vol[x+1, y, z]
    
    center_rough <- rough_vol[x, y, z]
    neighbor_rough <- rough_vol[x+1, y, z]
    
    if (sd(c(center_smooth, neighbor_smooth)) > 0) {
      smooth_corr[i] <- cor(center_smooth, neighbor_smooth)
    }
    if (sd(c(center_rough, neighbor_rough)) > 0) {
      rough_corr[i] <- cor(center_rough, neighbor_rough)
    }
  }
  
  # Average correlation should be higher for smoothed data
  # This is a weak test due to single timepoint, but checks basic behavior
  smooth_mean <- mean(smooth_corr[!is.na(smooth_corr)])
  rough_mean <- mean(rough_corr[!is.na(rough_corr)])
  
  # Skip test if all correlations are NA (shouldn't happen but safety check)
  if (!is.na(smooth_mean) && !is.na(rough_mean)) {
    expect_true(smooth_mean >= rough_mean)
  }
})
